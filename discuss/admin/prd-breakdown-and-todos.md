...
- 登录形态：PRD 提到“微信扫码/手机号验证码登录（客户）”，但也允许“选片链接直达”
- 建议 MVP：优先实现“选片链接 + 访问令牌”模式；客户登录作为增强（P1）

**后端**

- 员工登录：账号/密码 → JWT（或 Session）签发
- 客户登录（如做）：手机号验证码/扫码登录的 Provider 接入与风控
...
- 客户登录形态：链接直达 vs 登录后进入“我的选片台”（两种路径是否都做）
