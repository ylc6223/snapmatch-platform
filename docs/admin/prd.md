...
| V1.3 | 2026-01-10 | AI | **新增项目管理页面，废弃原"创建项目"：** 新增 4.4.1 项目管理章节，包含项目管理列表页的详细需求（路由 `/dashboard/delivery/projects/list`）；原 4.4.1 创建项目已废弃并用删除线标记；工作台快捷入口页面的"查看全部"按钮路由更新为项目管理列表页 |
...
**功能定位**: 管理所有摄影项目的列表页面，提供完整的项目增删改查功能。

**页面路由**: `/dashboard/delivery/projects/list`

**页面布局**:

1. **面包屑导航**: 显示当前位置（工作台 > 交付管理 > 项目管理）
2. **页面标题和操作按钮**:
   - 标题：项目管理
   - 描述：管理所有摄影项目，包括项目创建、照片上传、选片进度等
   - 操作按钮：新建项目（跳转到 `/dashboard/delivery/projects/new`）

3. **筛选工具栏** (复用工作台快捷入口页面的组件):
   - 状态筛选标签：全部、进行中、选片中、修图中、已交付
   - 排序功能：最新创建 / 最早创建
   - 视图切换：网格视图 / 列表视图

4. **项目网格展示** (复用工作台快捷入口页面的组件):
   - 以卡片网格形式展示所有项目
   - 支持筛选、排序、分页
   - 点击卡片跳转到项目详情页
