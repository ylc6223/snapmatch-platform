...
```javascript
const result = await db.collection('todos').doc('docId').get();
```
...
// Pagination
db.collection('posts').orderBy('createdAt', 'desc').skip(20).limit(10).get();

// Field selection
db.collection('users').field({ name: true, email: true, _id: false }).get();
```
